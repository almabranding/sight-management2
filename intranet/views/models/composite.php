         ...
         <?php
            foreach ($this->composite as $key => $value) {
                ...
                <li rel="<?= $value['photo_id'] ?>" id="foo_<?php echo $value['id']; ?>" class="ui-state-default listImage modelList <? echo ($value['main']) ? 'mainPic' : '' ?>" onclick="">
                    ...
                    <img caption="<?php echo $value['caption_' . LANG]; ?>" src="<?php echo URL . UPLOAD . 'models/' . Model::idToRute($value['photo_id']) . 'thumb_' . $value['file_file_name']; ?>"><a target="_blank" href="<?php echo URL . UPLOAD . $this->id . '/' . $value['img']; ?>">
                </li>
            <?php } ?>
    </ul>
    <div>
